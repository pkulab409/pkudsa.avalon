...
def record_duel(winner_team: str, winner_ids: list[int], player_ids: list[int]):
    """记录一场对局的结果"""
    
    # 检查玩家人数
    if len(player_ids) != 7:
        raise ValueError("必须传入7个玩家ID")

    # 检查胜利玩家ID是否与获胜方相关
    if len(winner_ids) == 0:
        raise ValueError("获胜方的玩家ID不能为空")
    ...
    try:
        cursor.execute('''
            INSERT INTO duels (
                ...
                winner_team, winner_ids
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            *player_ids,                # 7个玩家ID
            winner_team,                # 胜利方团队（如 "Good" 或 "Evil"）
            json.dumps(winner_ids)      # 胜利方玩家ID（JSON 格式）
        ))

        db.commit()
        logging.info(f"Successfully recorded duel: {player_ids} - Winner Team: {winner_team} - Winner IDs: {winner_ids}")

    except Exception as e:
        db.rollback()
        logging.error(f"Error recording duel: {e}")
        raise e