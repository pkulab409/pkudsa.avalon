...
class Player:
    ...
    def walk(self) -> tuple[str, ...]:
        ...
        for direction, dx, dy in directions:
            nx, ny = x + dx, y + dy
            if (0 <= nx < self.map_size) and (0 <= ny < self.map_size) and ((nx, ny) not in others_pos):
    ...
    def _find_path_to_target(self, x, y, target):
        ...
        visited =  set([(x, y)])
        others_pos = [self.player_positions[i] for i in range(1,8) if i != self.index]  #其他玩家位置
        ...
        while queue and bfs_time:
            ...
            for direction, dx, dy in directions:
                ...
                if (0 <= nx < self.map_size) and (0 <= ny < self.map_size) and ((nx,  ny) not in others_pos) and ((nx, ny) not in visited):
