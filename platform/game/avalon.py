...
class Player:
            ...
            if "梅林" in message or "merlin" in message.lower():
                ...
                if match:
                    ...
                    if not self.is_evil:
                        ...
                        # 如果邪恶方，记录被指认为梅林的玩家
                        write_into_private(
                            f"possible_merlin: {accused}, mentioned_by: {speaker}"
                        )
        except Exception as e:
            pass
    ...
    def pass_mission_members(self, leader: int, members: list[int]):
        ...
        try:
            # 记录到私有库
            write_into_private(
                f"round_{self.round}_team: leader={leader}, members={members}"
            )
        ...
        try:
            ...
            while len(members) < member_number:
                ...
                if candidates:
                    ...
                    candidates = [
                        i for i in range(1, self.player_count + 1) if i not in members
                    ]
                    if candidates:
                        members.append(random.choice(candidates))
    ...
    def mission_vote1(self) -> bool:
        ...
        try:
            ...
            # 邪恶方策略
                ...
                evil_count = sum(
                    1 for m in self.last_team if m in self.trusted or m == self.index
                )
                team_size = len(self.last_team)
    ...
    def assass(self) -> int:
        ...
        try:
            ...
            if matches:
                ...
                write_into_private(
                    f"assassinate_target: {target}, reason: LLM analysis"
                )
                return target

            # 如果LLM没给出明确答案，从怀疑度最低的玩家中选择
            suspects_in_game = (
                set(range(1, self.player_count + 1)) - self.suspects - {self.index}
            )
            if suspects_in_game:
                target = random.choice(list(suspects_in_game))
                write_into_private(
                    f"assassinate_target: {target}, reason: lowest suspicion"
                )
                return target
            else:
                # 随机选择非自己的玩家
                ...
                write_into_private(
                    f"assassinate_target: {target}, reason: random choice"
                )
                return target
