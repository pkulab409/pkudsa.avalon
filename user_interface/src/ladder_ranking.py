'''集成天梯排名功能，可以获取总天梯榜或分区天梯榜并在标签页界面中展示。'''
...
def get_overall_ladder() -> pd.DataFrame:
    '''返回总天梯榜。'''
...
def get_division_ladder(division) -> pd.DataFrame:
    '''返回指定分区的天梯榜。'''
    if not division:
        return pd.DataFrame(columns=["排名", "用户名", "天梯积分"])
        ...
        with gr.Row():
            with gr.Column():
                ...
                division_dropdown = gr.Dropdown(
                    ...
                    label="选择分区",
                    value=get_all_divisions()[
                        0] if get_all_divisions() else None
                )
                ...
                # 初始加载第一个分区的排行榜
                division_dropdown.value = get_all_divisions(
                )[0] if get_all_divisions() else None
